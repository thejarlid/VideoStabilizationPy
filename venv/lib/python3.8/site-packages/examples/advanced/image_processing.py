...
denoise_gradx, denoise_grady = list(zip(*denoise(img_gradx, img_grady, 0.2)))
...
gradx_obj = list(map(square, (fx - gx for fx, gx in zip(grad(new_img,'x'),denoise_gradx))))
grady_obj = list(map(square, (fy - gy for fy, gy in zip(grad(new_img,'y'),denoise_grady))))
